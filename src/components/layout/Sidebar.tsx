  ...
  const sidebarContent = (
    <div className="flex flex-col justify-between h-full">
      <div className="py-4 px-3 space-y-5 overflow-y-auto">
        {/* Quick Menu Search Shortcut Button */}
        {onOpenSearch && (
          <button
            id="sidebar-quick-search-btn"
            type="button"
            onClick={onOpenSearch}
            className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-amber-500/30 text-slate-300 hover:text-white transition-all cursor-pointer group text-xs shadow-xs"
            title="جستجو در تمام منوها و تنظیمات (Ctrl + K)"
          >
            <div className="flex items-center gap-2">
              <Search className="w-3.5 h-3.5 text-amber-400 group-hover:scale-110 transition-transform" />
              <span className="font-medium text-slate-300">جستجو در منوها...</span>
            </div>
            <kbd className="text-[10px] font-mono bg-white/10 text-slate-400 group-hover:text-amber-300 px-1.5 py-0.5 rounded border border-white/10 transition-colors">
              Ctrl+K
            </kbd>
          </button>
        )}
